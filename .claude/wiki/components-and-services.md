...
| `WorkoutService` | `users/{uid}/workouts` | `workouts` (`undefined` while loading) | `add`, `update`, `remove`, `stageGroupReassign(batch, from, to)`, `setNote(id, note, noteCreatedAt)` — a *targeted* two-field write for the exercise's [standing note](./database.md#note-carry-over), deliberately not routed through `update` (which takes a whole workout and would revert a `usualWeight` written moments earlier); module functions `isOrphanGroup`, `loggableGroups(groups, workouts)`, `workoutsInGroup(workouts, group, knownGroups)` — the last two shared by the Weeks logging modal and the set builder so their pickers can't drift |
| `WorkoutSetService` | `users/{uid}/workoutSets` | `sets` (`undefined` while loading) | `add`, `update`, `remove`. Saved **day sets** — reusable bundles of exercises. Items live in an array on the doc, sanitized on every write (Firestore rejects `undefined`, and these live *inside* an array — see [Database → workoutSets](./database.md#usersuidworkoutsetssetid)). Exports `sanitizeItem` so `WeekSetService` shares that guard rather than copying it |
| `WeekSetService` | `users/{uid}/weekSets` | `weekSets` (`undefined` while loading) | `add`, `update`, `remove`. Saved **week sets** — a whole Mon–Sun plan as `days: { day, items }[]`, reusing `SetItem` verbatim. Writes drop empty days, sort by `day`, and run every item through the shared `sanitizeItem` — the nesting is two arrays deep. Also exports `weekSetItemCount(weekSet)` for the `/sets` card stat |
| `WeekService` | `users/{uid}/weeks/{weekId}/entries` | `entries`, `currentWeekStart`, `weekId`, `rangeLabel`, `isCurrentWeek`, `today` | `add`, `update`, `remove`, `previousWeek`, `nextWeek`, `goToThisWeek`; module functions include the local-day-id codec `toWeekId`/`parseDateId`/`entryDate` and `formatDayId(id, today?)` ("Jun 16", or "Jun 16, 2025" across a year boundary — used for note origins), plus `entriesFor(uid, weekId)` — the same live query against *any* uid, used by the Friends page to show a friend's week (rules decide; the stream errors if they aren't friends) |
...
| `ExerciseAnalyticsService` | collection-group over all `entries` (+ `WorkoutService`/`SettingsService`) | `entries` (all weeks), `loaded`, `groups` | `exercisesInGroup(group)`, `sessionsFor(ids)`, `totalsEntries()` — read-only. `sessionsFor` takes an id list and drops `cardio`; `totalsEntries` takes none and keeps it, which is why totals couldn't be built on the former |
...
| `apply-set.ts` | `entriesFromSet` (a saved set → logged entries, with the skip/body-weight/no-write-back/note-precedence rules) and `setItemsFromEntries` (the inverse, for "save this day as a set"). All three apply functions take an optional `NoteLookup` — standing exercise notes by id — which fills an item's blank note; omit it and nothing is carried. |
...
| `SetFormModalComponent` | `app-set-form-modal` | `open`, `mode` (`'day'`\|`'week'`), `editingSet`, `presetItems`, `editingWeekSet`, `presetDays`, `presetName` | `close`, `saved`, `savedWeek` | Owns the whole create/edit-**set** builder: name, description, and an ordered list of `SetItemEditorComponent` blocks. Same input-driven shape as `WorkoutFormModalComponent`, and re-seeds on the closed→open transition the same way. `presetItems`/`presetDays`/`presetName` open it pre-filled — that's the Weeks page's "save this day as a set" and "save this week as a set". `mode: 'week'` renders the same blocks under seven collapsible weekday sections and writes to `WeekSetService`; the blocks stay one flat list, each `BuilderItem` carrying a builder-only `day`. Layers `WorkoutFormModalComponent` over itself for "+ Create new workout". The only `wide` modal: from 900px up the exercise blocks lay out as columns wrapping onto the next row (four across at full width — the count falls out of a 320px track floor against the 1500px cap, so it can't overflow sideways), and name/description pair up; below 900px it's the plain single-column stack. See [Features → Workout Sets](./features.md#workout-sets-reusable-days-and-weeks). |
...
| `TrainingTotalsComponent` | `app-training-totals` | `range` (required) | The third Analytics card: lifting tiles, cardio tiles, and a breakdown table toggled between per-exercise and per-muscle-group. Reads `ExerciseAnalyticsService.totalsEntries()`, windows it with `rangeStart`, and folds it with the pure `computeTotals` (`analytics/totals.ts`). Converts the canonical **sum** once at the display boundary and formats with `toLocaleString()` — not the `lifted` pipe, which emits ungrouped digits. Uses no chart component. |
